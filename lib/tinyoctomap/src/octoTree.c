...
void bresenham3D(octoTree_t *octoTree, octoMap_t *octoMap, coordinate_t *start, coordinate_t *end)
{
    ...
    int steepXY = (abs(item_end.y - item_start.y) > abs(item_end.x - item_start.x));
    if (steepXY) {
        int temp = item_start.x;
    ...
    int steepXZ = (abs(item_end.z - item_start.z) > abs(item_end.x - item_start.x));
    if (steepXZ) {
        int temp = item_start.x;
    ...
    int deltaX = abs(item_end.x - item_start.x);
    int deltaY = abs(item_end.y - item_start.y);
    int deltaZ = abs(item_end.z - item_start.z);

    int errorXY = deltaX / 2;
    int errorXZ = deltaX / 2;

    int stepX = item_start.x < item_end.x ? TREE_RESOLUTION : -TREE_RESOLUTION;
    int stepY = item_start.y < item_end.y ? TREE_RESOLUTION : -TREE_RESOLUTION;
    int stepZ = item_start.z < item_end.z ? TREE_RESOLUTION : -TREE_RESOLUTION;
